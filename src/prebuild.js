  ...
  var atlasCreator, block, buildPath, config, fs, i, j, k, maxStateId, pBlock, ref, result;
  ...
  for (i = k = 0, ref = maxStateId; (0 <= ref ? k <= ref : k >= ref); i = 0 <= ref ? ++k : --k) {
    block = pBlock.fromStateId(i);
    result.push([block.name, block.boundingBox === "block" ? 1 : 0, block.transparent ? 1 : 0]);
  }
